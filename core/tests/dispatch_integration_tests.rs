...
fn create_and_run_lazy_pinger(
    system: &KompactSystem,
    target_ponger: &ActorPath,
) -> (Arc<Component<PingerAct>>, KFuture<()>) {
    let mut pinger_act = PingerAct::new_lazy(target_ponger.clone());
    let pong_future = pinger_act.make_future();
    let (pinger, reg_future) = system.create_and_register(move || pinger_act);
    reg_future.wait_expect(Duration::from_millis(1000), "Pinger failed to register!");
    system.start(&pinger);
    (pinger, pong_future)
}

fn create_and_run_eager_pinger(
    system: &KompactSystem,
    target_ponger: &ActorPath,
) -> (Arc<Component<PingerAct>>, KFuture<()>) {
    let mut pinger_act = PingerAct::new_eager(target_ponger.clone());
    let pong_future = pinger_act.make_future();
    let (pinger, reg_future) = system.create_and_register(move || pinger_act);
    reg_future.wait_expect(Duration::from_millis(1000), "Pinger failed to register!");
    system.start(&pinger);
...
fn create_and_run_lazy_ponger(system: &KompactSystem) -> (Arc<Component<PongerAct>>, ActorPath) {
...
fn create_and_run_eager_ponger(system: &KompactSystem) -> (Arc<Component<PongerAct>>, ActorPath) {
    let (ponger, pof) = system.create_and_register(PongerAct::new_eager);
    let ponger_path = system.actor_path_for(&ponger);
    pof.wait_expect(Duration::from_millis(1000), "Ponger failed to register!");
    system.start(&ponger);
    (ponger, ponger_path)
...
fn create_and_run_status_counter(system: &KompactSystem) -> Arc<Component<NetworkStatusCounter>> {
    let (status_counter, _) = system.create_and_register(NetworkStatusCounter::new);
...
fn create_and_run_big_pinger(
    system: &KompactSystem,
    mut big_pinger: BigPingerAct,
) -> (Arc<Component<BigPingerAct>>, KFuture<()>) {
    let future = big_pinger.make_future();
...
fn remote_delivery_to_registered_actors_eager() {
    ...
    let (ponger_unique, ponger_unique_path) = create_and_run_eager_ponger(&remote);
    let (ponger_named, _) = create_and_run_eager_ponger(&remote);
    let ponger_named_path = remote
        .register_by_alias(&ponger_named, "custom_name")
        .wait_expect(Duration::from_millis(1000), "Ponger failed to register!");

    let (pinger_unique, pinger_done_future1) =
        create_and_run_eager_pinger(&system, &ponger_unique_path);
    let (pinger_named, pinger_done_future2) =
        create_and_run_eager_pinger(&system, &ponger_named_path);
...
fn remote_delivery_bigger_than_buffer_messages_lazy_tcp() {
    ...
    let (ponger, ponger_path) =
        create_and_run_big_ponger(&system, BigPongerAct::new_lazy());

    let (pinger, pinger_complete_future) =
        create_and_run_big_pinger(&remote, BigPingerAct::new_lazy(ponger_path, 120));

    pinger_complete_future.wait_timeout(Duration::from_secs(10)).expect("Should complete");

    remote.stop_notify(&pinger)
        .wait_timeout(Duration::from_secs(1))
        .expect("Pinger never stopped!");
    system.kill_notify(ponger)
        .wait_timeout(Duration::from_secs(1))
        .expect("Ponger never died!");
...
fn remote_delivery_bigger_than_buffer_messages_eager_tcp() {
    ...
    let (ponger, ponger_path) =
        create_and_run_big_ponger(&system, BigPongerAct::new_eager(buf_cfg.clone()));

    let (pinger, pinger_complete_future) =
        create_and_run_big_pinger(&remote, BigPingerAct::new_eager(ponger_path, 120, buf_cfg));
...
fn remote_delivery_bigger_than_buffer_messages_preserialised_tcp() {
    ...
    let (pinger, pinger_complete_future) =
        create_and_run_big_pinger(&remote, BigPingerAct::new_preserialised(ponger_path, 120, buf_cfg));
...
fn remote_delivery_bigger_than_buffer_messages_lazy_udp() {
    ...
    ponger_path.via_udp();
    let (pinger, pinger_complete_future) =
        create_and_run_big_pinger(&remote, BigPingerAct::new_lazy(ponger_path, 1500 * 3 / 4));
...
fn remote_delivery_bigger_than_buffer_messages_eager_udp() {
    ...
    ponger_path.via_udp();
    let (pinger, pinger_complete_future) =
        create_and_run_big_pinger(&remote,BigPingerAct::new_eager(ponger_path, 1500 * 3 / 4, buf_cfg));
...
fn remote_delivery_bigger_than_buffer_messages_preserialised_udp() {
    ...
    ponger_path.via_udp();
    let (pinger, pinger_complete_future) =
        create_and_run_big_pinger(&remote,BigPingerAct::new_preserialised(ponger_path, 1500 * 3 / 4, buf_cfg));
...
fn remote_delivery_to_registered_actors_eager_mixed_udp() {
    ...
    let (ponger_unique, mut ponger_unique_path) = create_and_run_eager_ponger(&remote);
    let (ponger_named, _) = create_and_run_eager_ponger(&remote);
    let mut ponger_named_path = remote
        .register_by_alias(&ponger_named, "custom_name")
        .wait_expect(Duration::from_millis(1000), "Ponger failed to register!");
...
fn remote_delivery_to_registered_actors_lazy() {
    ...
    let (ponger_unique, ponger_unique_path) = create_and_run_lazy_ponger(&remote);
    let (ponger_named, _) = create_and_run_lazy_ponger(&remote);
    let ponger_named_path = remote
        .register_by_alias(&ponger_named, "custom_name")
        .wait_expect(Duration::from_millis(1000), "Ponger failed to register!");

    let (pinger_unique, pinger_done_future1) =
        create_and_run_lazy_pinger(&system, &ponger_unique_path);
    let (pinger_named, pinger_done_future2) =
        create_and_run_lazy_pinger(&system, &ponger_named_path);
...
fn remote_delivery_to_registered_actors_lazy_mixed_udp() {
    ...
    let (ponger_unique, mut ponger_unique_path) = create_and_run_lazy_ponger(&remote);
    let (ponger_named, _) = create_and_run_lazy_ponger(&remote);
    let mut ponger_named_path = remote
        .register_by_alias(&ponger_named, "custom_name")
        .wait_expect(Duration::from_millis(1000), "Ponger failed to register!");
...
fn remote_lost_and_continued_connection() {
    ...
    let (ponger_named, _) = create_and_run_lazy_ponger(&remote_a);
    remote_a
        .register_by_alias(&ponger_named, "custom_name")
        .wait_expect(Duration::from_millis(1000), "Ponger failed to register!");
    ...
    let (pinger1, pinger_done_future1) = create_and_run_lazy_pinger(&system, &named_path);
    pinger_done_future1
        .wait_timeout(Duration::from_secs(10))
    ...
    let (pinger2, pinger_done_future2) = create_and_run_lazy_pinger(&system, &named_path);
    ...
    let (ponger_named, _) = create_and_run_lazy_ponger(&remote_b);
    remote_b
        .register_by_alias(&ponger_named, "custom_name")
        .wait_expect(Duration::from_millis(1000), "Ponger failed to register!");
    ...
    let pingfu = system.stop_notify(&pinger1);
    let pingfn = system.stop_notify(&pinger2);
    let pongfn = remote_b.kill_notify(ponger_named);
...
fn remote_lost_and_dropped_connection() {
    ...
    let (pinger_named, pinger_done_future1) = create_and_run_lazy_pinger(&system, &named_path);
    pinger_done_future1
        .wait_timeout(Duration::from_secs(10))
        .expect("Pings should complete!");
    ...
    // Start a new pinger on system
    let (pinger_named2, _) = create_and_run_lazy_pinger(&system, &named_path);
    // Wait for it to send its pings, system should recognize the remote address
    ...
    let (pinger_named3, pinger_done_future3) = create_and_run_lazy_pinger(&system, &named_path);
...
fn local_delivery() {
    ...
    let (ponger, mut ponger_path) = create_and_run_lazy_ponger(&system);
    ponger_path.set_protocol(Transport::Local);

    let (pinger, pinger_done_future) = create_and_run_lazy_pinger(&system, &ponger_path);
    pinger_done_future
        .wait_timeout(Duration::from_secs(10))
        .expect("Pings should complete!");
...
fn local_forwarding() {
    ...
    let (pinger, pinger_complete_future) =
        create_and_run_big_pinger(&system,BigPingerAct::new_lazy(forwarder_path, 512));
    ...
    system.kill_notify(ponger)
        .wait_timeout(Duration::from_secs(1))
        .expect("Ponger never died!");
    system.kill_notify(forwarder)
        .wait_timeout(Duration::from_secs(1))
        .expect("Forwarder never died!");
    system.stop_notify(&pinger)
        .wait_timeout(Duration::from_secs(1))
        .expect("Pinger never stopped!");
...
fn local_forwarding_eager() {
    ...
    let (pinger, pinger_complete_future) =
        create_and_run_big_pinger(&system,BigPingerAct::new_eager(forwarder_path, 512, BufferConfig::default()));
...
fn remote_forwarding_unique() {
    ...
    let (ponger, ponger_path) =
        create_and_run_big_ponger(&system1, BigPongerAct::new_lazy());
    ...
    let (pinger, pinger_complete_future) =
        create_and_run_big_pinger(&system3,BigPingerAct::new_lazy(forwarder_path, 512));

    pinger_complete_future.wait_timeout(Duration::from_secs(10)).expect("Should complete");

    system1.kill_notify(ponger)
        .wait_timeout(Duration::from_secs(1))
        .expect("Ponger never died!");
    system2.kill_notify(forwarder)
        .wait_timeout(Duration::from_secs(1))
        .expect("Forwarder never died!");
    system3.stop_notify(&pinger)
        .wait_timeout(Duration::from_secs(1))
        .expect("Pinger never stopped!");
...
fn remote_forwarding_unique_two_systems() {
    ...
    let (pinger, pinger_complete_future) =
        create_and_run_big_pinger(&system2,BigPingerAct::new_lazy(forwarder_path, 512));
...
fn remote_forwarding_named() {
    ...
    let ponger_path = system1.register_by_alias(&ponger, "ponger")
        .wait_expect(Duration::from_millis(1000), "Ponger failed to register!");

    let (forwarder, _fof) = system2.create_and_register(move || ForwarderAct::new(ponger_path));
    let forwarder_path = system2.register_by_alias(&forwarder, "forwarder")
        .wait_expect(Duration::from_millis(1000), "Forwarder failed to register!");
    system2.start(&forwarder);
...
fn network_status_port_established_lost_dropped_connection() {
    ...
    let status_counter = create_and_run_status_counter(&local_system);

    // Create a pinger ponger pair such that the Network will be used.
    let (_, ponger_path) = create_and_run_lazy_ponger(&remote_system);
    let (_, pinger_done_future) = create_and_run_lazy_pinger(&local_system, &ponger_path);
    ...
    let (pinger2, reg_future) =
        local_system.create_and_register(|| PingerAct::new_lazy(ponger_path.clone()));
    reg_future.wait_expect(Duration::from_millis(1000), "Pinger failed to register!");
    local_system.start(&pinger2);

    thread::sleep(Duration::from_millis(2000));
    // Make sure the local_system discovers the lost connection on Linux....
    let (pinger3, reg_future) =
        local_system.create_and_register(|| PingerAct::new_lazy(ponger_path.clone()));
    reg_future.wait_expect(Duration::from_millis(1000), "Pinger failed to register!");
    local_system.start(&pinger3);
